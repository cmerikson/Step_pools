...
# Numerical Solver
function Gaussian_DualPeak_Model(Peaks::Vector, Discharge::Float64, Roughness_Scale::Float64, Length_Scale::Float64,
     D50::Float64, Slope::Float64, timestep::Float64, simtime::Float64, ν::Float64;
     u_init::Float64=1.0, τ_cr::Union{Nothing, Float64}=nothing, σ::Float64=0.1, A::Float64=2.0,
     Reference::Union{String, Nothing}=nothing, figure::Bool=false, flat::Bool=false, k::Float64=3.0, transport_intesity::Float64=1.5, domain::Float64=10.0, nodes::Int64=250)
    ...
    for i in eachindex(Peaks)
        if !flat
            CurrentPeak = (A/r_0)*exp((-(x - Peaks[i])^2.0)/(2*σ^2.0))
            r += CurrentPeak
        end
    end
    ...
    domains = [x ∈ Interval(x_start,x_end), t ∈ Interval(t_start,t_end)]
    ...
    if length(Peaks) > 0
        for i in eachindex(Peaks)
            CurrentPeak = (A/r_0).*exp.((-(discx .- Peaks[i]).^2.0)./(2*σ^2.0))
            discr = discr .+ CurrentPeak
        end
    end
...
function run_simulations(Peaks::Vector, Discharge::Float64, Roughness_Scale::Float64, Length_Scale::Float64, D50::Float64, Slope::Float64, timestep::Float64, simtime::Float64, ν; u_init::Float64=1.0, τ_cr::Float64=0.05, Reference::String=nothing)
    results = Gaussian_DualPeak_Model(Peaks, Discharge, Roughness_Scale, Length_Scale, D50, Slope, timestep, simtime, ν, u_init=u_init, τ_cr=τ_cr, Reference=Reference)
    return vcat(results...)
end
